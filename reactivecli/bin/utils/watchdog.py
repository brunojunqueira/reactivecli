...
class Watcher:
	...
	def __init__(self, target, on_change):
		...
		try:
			...
			self.target_callback = self._private_target_module.__getattribute__(self._private_target_name)
		except Exception as e:
			error_label = str(e).replace(f"{self._private_target_file_name}, ", f"{self._private_target_path}, ")
	...
	def start(self):
		t = threading.Thread(target=self._private_wait_changes, args=(), daemon=True)
		t.start()
